...
from db_operations import add_documents
...
# TODO: chunk on paragraphs, instead of number of words
...
# # TODO: add caching and fliewatching to rebuild index
...
#             with open(os.path.join(folder_path, filename), 'r', encoding="utf-8", errors='replace') as f:
    ...
    for filename in os.listdir(folder_path):
        ...
        if filename.endswith(".txt"):
            with open(os.path.join(folder_path, filename), 'r', encoding="utf-8", errors='replace') as f:
                ...
                add_documents(chunks, None, [f"{filename}_{i}" for i in range(len(chunks))])


# TODO: PDF to text document (cache it, so if the file is already processed, don't process it again)
def pdf_to_text():
    # print(os.listdir("."))
    for file in os.listdir('./pdfs/'):
        if (file.endswith('.pdf') and 
            not os.path.exists('./docs/' + file.replace('.pdf', '.txt'))
        ):
            reader = PdfReader(os.path.join(os.curdir, "pdfs", file))
            text_body = ""
            try:
                for page in reader.pages:
                    text_body += page.extract_text()
            except:
                text_body = fallback_text_extraction(os.path.join(os.curdir, "pdfs", file))

            with open('./docs/' + file.replace('.pdf', '.txt'), 'xt', errors='replace') as f:
                f.write(text_body)
